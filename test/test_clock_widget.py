"""Pytest suite closely mirroring the original Rust tests.
...
def test_clock_pid_angles_in_radians():
    ...
    assert approx_eq(s_rad, math.pi / 2.0, 1e-6)
    assert approx_eq(m_rad, math.pi, 1e-6)
    assert approx_eq(h_rad, math.pi, 1e-6)


def test_angles_zero_half_full_quarter():
...
def test_polar_to_cartesian_zero_and_quarters():
    center = QPointF(100.0, 100.0)
    res = polar_to_cartesian(center, 50.0, 0.0)
    assert abs(res.x() - center.x()) < 1e-6
    assert abs(res.y() - (center.y() - 50.0)) < 1e-6

    center = QPointF(0.0, 0.0)
    res = polar_to_cartesian(center, 1.0, math.pi / 2.0)
    assert abs(res.x() - 1.0) < 1e-6
    assert abs(res.y() - 0.0) < 1e-6
