...
from src.gui_setup import create_moc, UiExtensions
...
def test_create_moc_ui(temp_dir, example_moc_content_ui) -> None:
...
def test_create_moc_qrc(temp_dir, example_moc_content_qrc) -> None:
...
def test_create_moc_check_if_correctly_modified_existing_code(temp_dir, example_moc_content_ui) -> None:
    input_file = os.path.join(temp_dir, "example.ui")
    with open(input_file, "w") as f:
...
def test_create_moc_error_when_create_qrc_contnent_for_ui(temp_dir,example_moc_content_qrc) -> None:
    input_file = os.path.join(temp_dir, "example.ui")
    with open(input_file, "w") as f:
        f.write(example_moc_content_qrc)
...
def test_create_moc_error_when_create_ui_content_for_qrc(temp_dir, example_moc_content_ui) -> None:
    input_file = os.path.join(temp_dir, "example.ui")
    with open(input_file, "w") as f:
        f.write(example_moc_content_ui)
